	...
	function trav_acc_get_room_detail_html( $room_type_id, $type = 'all', $room_price = 0, $number_of_days = 0, $rooms = 0) { // available type - all,available,not_available,not_match
		...
		<article class="box">
			<figure class="col-sm-4 col-md-3">
				<a class="hover-effect" data-post_id="<?php echo esc_attr( $room_type_id );?>" href="<?php echo esc_url( get_permalink( $acc_id ) ); ?>" title="<?php echo __( 'popup gallery', 'trav' ); ?>"><?php echo get_the_post_thumbnail( $room_type_id, 'list-thumb' ); ?></a>
			</figure>
			<div class="details col-xs-12 col-sm-8 col-md-9">
				<div>
					<div>
						<div class="box-title">
							<h4 class="title"><a href="<?php echo esc_url( get_permalink( $acc_id ) ); ?>"><?php echo esc_html( get_the_title( $room_type_id ) ); ?></a></h4>
...
if ( ! function_exists( 'trav_acc_get_acc_list_sigle' ) ) {
	function trav_acc_get_acc_list_sigle( $acc_id, $list_style, $before_article='', $after_article='', $show_badge=false, $animation='' ) {
		...
		$address = get_post_meta( $acc_id, 'trav_accommodation_address', true );
		...
		if ( $list_style == "style1" || $list_style == "style2" ) { ?>
			<article class="box">
				<figure <?php echo wp_kses_post( $animation ) ?>>
					<a href="<?php echo esc_url( get_permalink( $acc_id ) ); ?>" data-post_id="<?php echo esc_attr( $acc_id ) ?>" class="hover-effect"><?php echo get_the_post_thumbnail( $acc_id, 'biggallery-thumb' );  ?></a>
				...
				<div class="details">
					<?php if ( $list_style == "style1" ) {
						if ( ! empty( $avg_price ) && is_numeric( $avg_price ) ) { ?>
							<span class="price"><small><?php _e( 'mỗi đêm', 'trav' ) ?></small><?php echo esc_html( trav_get_price_field( $avg_price ) ); ?></span>
						<?php } ?>
						<h4 class="box-title">
					...
					<?php } elseif ( $list_style == "style2" ) { ?>
						<?php if ( ! empty( get_field('price_per_person') ) && is_numeric( get_field('price_per_person') ) ) { ?>
							<span class="price"><?php echo esc_html( ho_format_money(get_field('price_per_person'))  ); ?></span>
						<?php } ?>
						<h4 class="box-title"><a href="<?php echo esc_url( get_permalink( $acc_id ) ); ?>"><?php echo esc_html( get_the_title( $acc_id ) ); ?></a></h4>
						<div class="feedback">
							<div data-placement="bottom" data-toggle="tooltip" class="five-stars-container" title="<?php echo esc_attr( $review . ' ' . __( 'stars', 'trav' ) ) ?>"><span style="width: <?php echo esc_attr( $review / 5 * 100 ) ?>%;" class="five-stars"></span></div>
							<span class="review"><?php echo get_post_meta($acc_id, 'trav_accommodation_star', true); ?></span>
						</div>
						<p class="address"><?php echo $address; ?></p>
						<div class="action clearfix">							
							<a class="button btn-small" href="<?php echo esc_url( get_permalink( $acc_id ) );  ?>">Chi tiết</a>
							<a class="button btn-small yellow popup-map" href="#" data-box="<?php echo esc_attr( $loc ) ?>">Đặt phòng</a>							
						</div>
