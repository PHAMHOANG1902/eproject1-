...
if ( ! function_exists( 'trav_tour_get_special_tours' ) ) {
    function trav_tour_get_special_tours( $type='latest', $count=10, $exclude_ids=array(), $country='', $city='', $tour_type=array() ) {
        ...
        if ( $type == 'featured'  ) {
            ...
            if ( ! empty( $city ) ) {
                $sql .= " INNER JOIN {$tbl_postmeta} AS meta_c2 ON (meta_c2.meta_key = 'trav_tour_city') AND (booking.tour_id = meta_c2.post_id)";
...
if ( ! function_exists( 'trav_tour_get_search_result' ) ) {
    function trav_tour_get_search_result( $search_data=array() ) { 
        ...
        $max_price = esc_sql( empty( $search_data['max_price'] ) ? '' : $search_data['max_price'] );
		...
		$place_from = esc_sql( empty( $search_data['place_from'] ) ? '' : $search_data['place_from'] );
		
		$place_to = esc_sql( empty( $search_data['place_to'] ) ? '' : $search_data['place_to'] );
	...
	if($date_from) {
		...
		for($i = 0; $i < 5;$i++){				
			...
			$tmp[] = array(
					'key' => 'tour_date_start_repeater_'.$i.'_tour_date_start',
					'value' => $date_from,
					'compare' => '='
			);
		}
		...
		// $tmp =  array(
					// 'key' => 'tour_date_start_repeater_0_tour_date_start',
					// 'value' => $date_from,
					// 'compare' => '='
			// );
