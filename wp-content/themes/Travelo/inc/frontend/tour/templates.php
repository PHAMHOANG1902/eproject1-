					...
					<div class="detail-section-top row">
						...
						<div class="price-details col-md-3 col-sm-4">
							...
							<?php if ( empty( $multi_book ) && empty( $repeated ) ) : ?>
								<button title="book now" class="button btn-small full-width text-center btn-book-now <?php echo empty( $default_data['available_seat'] ) ? 'no-display' : '' ?>"><?php _e( "BOOK NGAY", "trav" ); ?></button>
						...
						<div class="detail-section-bottom">
							<div class="row">
								...
								<div class="col-md-4 pull-right">
									...
									<div class="row">
										<div class="col-sm-12">
											<button data-animation-duration="1" data-animation-type="bounce" class="btn-book-now full-width icon-check animated bounce <?php echo empty( $default_data['available_seat'] ) ? 'no-display' : '' ?>" type="submit"><?php _e( "BOOK NGAY", "trav" ); ?></button>
											<h4 class="sold-out <?php echo empty( $default_data['available_seat'] ) ? '' : 'no-display' ?>"><?php echo __( 'Sold Out', 'trav' ) ?></h4>
											<h4 class="exceed-persons no-display"><?php echo __( 'Exceed Persons', 'trav' ) ?></h4>
										</div>
									</div>
	...
	function trav_tour_get_tour_list_sigle( $tour_id, $list_style, $before_article='', $after_article='', $show_badge=false, $animation='' ) {
			...
			<article class="box">
				...
				<div class="details">
					<?php if ( ! empty( $min_price ) && is_numeric( $min_price ) ) { ?>
						<span class="price"><?php echo ho_format_money(get_field('price_per_person', $tour_id)) ?></span>
					<?php } ?>
		...
		<?php } elseif ( $list_style == "style2" ) { ?>
			<article class="box">
				...
				<div class="details">
					<?php if ( ! empty( $min_price ) && is_numeric( $min_price ) ) { ?>
						<span class="price 12212"><?php echo ho_format_money(get_field('price_per_person', $tour_id)) ?></span>
					<?php } ?>
					<h4 class="box-title"><a href="<?php echo esc_url( $url ); ?>"><?php echo esc_html( get_the_title( $tour_id ) );?></a></h4>
					<hr>
					<div class="text-center">
			...
			<article class="box">
				...
				<div class="details col-sm-7 col-md-8">
					<div>
						...
						<div>
							<span class="price"><small>mỗi người</small><?php echo esc_html( trav_get_price_field( $min_price ) ); ?></span>
						</div>
					</div>
					<div>
						<?php echo wp_kses_post( $brief ); ?>
						<div>
							<a title="<?php _e( 'View Detail', 'trav' ); ?>" class="button btn-small full-width text-center" href="<?php echo esc_url( $url ); ?>"><?php _e( 'BOOK NGAY', 'trav' ); ?></a>
						</div>
					</div>
				</div>
			</article>
